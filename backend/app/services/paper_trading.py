...
async def execute_signal_trade(db: Session, signal: FundSignal) -> VirtualTrade | None:
    ...
    db.refresh(trade)
    logger.info(
        "가상 매수: %s %d주 @ %d원 (투자금: %d원, 수수료: %d원, 총비용: %d원)",
        stock.name, quantity, entry_price, invest_amount, buy_commission, total_buy_cost,
    )
    return trade


async def execute_pending_fund_signals(db: Session) -> dict:
    """오늘 생성된 미체결 펀드 시그널을 현재가(시장 시가)로 일괄 체결한다.

    09:05 KST 스케줄러에 의해 호출된다.
    신호 생성(08:30)과 체결(09:05)을 분리하여 실제 장중 시가 기준 시뮬레이션을 구현한다.

    Returns: {"buy_executed": int, "sell_executed": int, "skipped": int}
    """
    from app.services.naver_finance import fetch_current_price

    # KST 기준 오늘 시작: 08:30 KST 브리핑 신호(= 전날 23:30 UTC)가 today_start 이후로 잡히도록 KST 사용
    _KST = ZoneInfo("Asia/Seoul")
    today_start = datetime.combine(datetime.now(_KST).date(), datetime.min.time(), tzinfo=_KST)

    # 오늘 생성된 미체결 신호 (hold 제외)
    pending_signals = (
        db.query(FundSignal)
        .filter(
            FundSignal.created_at >= today_start,
            FundSignal.paper_executed.is_(False),
            FundSignal.signal.in_(["buy", "sell"]),
        )
        .order_by(FundSignal.created_at.asc())
        .all()
    )
    ...
    for signal in pending_signals:
        try:
            # 현재가 조회 (장 시작 시가에 최대한 근접)
            stock = db.query(Stock).filter(Stock.id == signal.stock_id).first()
            if stock:
                try:
                    current_price = await fetch_current_price(stock.stock_code)
                    if current_price and current_price > 0:
                        signal.price_at_signal = current_price
                except Exception:
                    pass  # 조회 실패 시 기존 price_at_signal 유지

            trade = await execute_signal_trade(db, signal)
            ...
            logger.error("펀드 시그널 배치 체결 실패 (id=%d): %s", signal.id, e)
            skipped += 1
        finally:
            # 체결 시도 여부와 무관하게 paper_executed=True로 마킹 (재시도 방지)
            signal.paper_executed = True

    db.commit()
    logger.info(
        "펀드 시그널 배치 체결 완료: 매수=%d, 매도=%d, 스킵=%d",
        buy_executed, sell_executed, skipped,
    )
    return {"buy_executed": buy_executed, "sell_executed": sell_executed, "skipped": skipped}
...
def _close_trade(
    ...
    trade.is_open = False
    trade.pnl = net_proceeds - total_cost
    trade.return_pct = round(trade.pnl / total_cost * 100, 2) if total_cost > 0 else 0.0

    # 현금 반환 (수수료/거래세 차감 후 순액)
