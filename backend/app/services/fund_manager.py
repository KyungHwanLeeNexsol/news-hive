...
아래 데이터의 각 필드 의미:
- current_price: 현재가, change_rate: 등락률(%), volume: 거래량
...
4. 하락장에서 함부로 매수 추천 금지. 시장 전체가 약세이면 대부분 "관망"이 맞습니다.
5. target_price(목표가)는 현재가 대비 +5%~+20% 범위, stop_loss(손절가)는 현재가 대비 -3%~-10% 범위에서 설정.
6. stock_picks는 3-5개 종목, "적극매수"는 최대 1개만 가능.
7. sector_highlights는 3-5개 섹터 배열.
8. 다시 한번 강조: 한자(漢字) 절대 사용 금지. 순수 한글만 사용하세요.
"""
