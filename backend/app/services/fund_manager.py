...
**핵심 규칙 (절대 위반 금지):**
1. stock_picks는 반드시 위 '후보 종목 데이터'의 수치를 직접 인용하여 분석하세요. 데이터 없이 추측 금지.
2. 뉴스에 언급됐다는 이유만으로 추천하지 마세요. 밸류에이션 + 수급 + 재무를 종합 판단하세요.
3. PER이 업종 평균 대비 50% 이상 높거나, ROE가 5% 미만이거나, 외국인+기관 동반 매도 중이면 "관망" 또는 "회피"로 설정.
4. **당일 급락 종목 매수 추천 절대 금지**: change_rate(당일 등락률)이 -3% 이하인 종목은 절대 "매수"나 "적극매수"로 설정하지 마세요. 반드시 "관망" 또는 "회피"로 설정하세요. 급락 중인 종목을 매수 추천하는 것은 투자자에게 큰 손실을 줍니다.
5. **당일 등락률(change_rate)을 반드시 확인**: 각 후보 종목의 change_rate 값을 먼저 확인하고, 하락 중인 종목은 그 이유를 분석하세요.
6. 하락장에서 함부로 매수 추천 금지. 시장 전체가 약세이면 대부분 "관망"이 맞습니다.
7. target_price(목표가)는 현재가 대비 +5%~+20% 범위, stop_loss(손절가)는 현재가 대비 -3%~-10% 범위에서 설정.
8. stock_picks는 3-5개 종목, "적극매수"는 최대 1개만 가능.
9. sector_highlights는 3-5개 섹터 배열.
10. 다시 한번 강조: 한자(漢字) 절대 사용 금지. 순수 한글만 사용하세요.
"""
