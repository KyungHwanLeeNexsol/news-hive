...
**목적**: `_gather_surge_candidates()`가 감싸는 동기 `gather_surge_candidates` 호출에 적용된
글로벌 타임아웃(300s=5분)이 코드베이스가 문서화한 정상 실행 시간(12~15분)의 절반에도 못 미쳐, 
유효한 탐지 결과를 타임아웃으로 조용히 폐기하던 버그 수정. 프로덕션에서 최근 ~11거래일 중 7일
(07-09/10/13/14/15/16/20) 영향 확인.

- **근본 원인**: 타임아웃 값 `_GATHER_TIMEOUT_S = 300`이 함수 내부 리터럴로 하드코딩되어 있어, 
  정상 실행(12~15분)을 크게 초과하는 날마다 `asyncio.TimeoutError` 발생 → `[]` 반환(오폐기)
- **구현**:
  1. 함수-로컬 리터럴 → 모듈 레벨 상수 승격(테스트 가능성 확보, REQ-004)
  2. 타임아웃 값 상향: 300s → 1200s(20분, 정상 상단 15분 대비 +33% 헤드룸)
  3. 타임아웃 경로는 기존 거동 보존(경고 로그 + 빈 리스트 반환, REQ-007)
- 신규 테스트 7개(245줄): DDD RED→GREEN 재현 증거 완비. 기존 mock 테스트 무회귀
  (test_surge_ai080_fund_manager.py 기존 케이스 동일 통과)
- 전체 회귀 스위트 통과(2008 passed, 0 failed). ruff clean.

**알려진 미해결 위험 [R-3]**: 타임아웃 후 고아 스레드의 세션 수명 안전성(취소 불가 스레드가 
공유 `db` 세션을 뒤늦게 건드리는 경쟁). 값 상향으로 발생 빈도는 감소하나 위험 자체는 닫히지 않음.
근본 수정(세션 격리 / 취소 가능 executor / thread-safe 핸드오프)은 후속 SPEC 후보로 유예.

**배포 상태**: 로컬 검증 완료(2026-07-20, commit b2e6cbc), main push 대기.

**영향 파일**: `backend/app/services/fund_manager.py`,
`backend/tests/test_surge_ai082_gather_timeout.py` (신규)
